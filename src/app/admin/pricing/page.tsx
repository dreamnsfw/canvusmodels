...
export default async function AdminPricingPage() {
  ...
  const models = await prisma.model.findMany({ orderBy: { type: "asc" } });

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold">Pricing</h1>
        <p className="mt-1 text-white/50">Configure credit costs per model</p>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/[0.06]">
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-white/40">Current Cost</th>
            ...
            {models.map((model) => (
              <tr key={model.id} className="border-b border-white/[0.04] last:border-0 hover:bg-white/[0.02] transition-colors">
                ...
                <td className="px-4 py-3 text-sm">
                  <Badge variant="premium">{model.creditsCost} credits</Badge>
                </td>
